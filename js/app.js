...
function setColor(id,color){
  ...
  if(p && img && p.images && p.images[color]){
    img.src=p.images[color];
  }
}
...
function addToCart(id){
  ...
  cart.push({name:p.name,price:p.price,size,color});
  save();
...
function renderCart(){
  ...
  cartBox.innerHTML=`<h3>Your Cart</h3>${cart.length?cart.map(i=>`<div class="cartrow"><span>${i.name}<br><small>${i.color?i.color+' • ':''}${i.size}</small></span><b>${money(i.price)}</b></div>`).join(''):'<p>Your cart is empty.</p>'}<p><b>Total: ${money(total)}</b></p><button class="add" onclick="cart=[];save()">Clear Cart</button><a class="btn primary" href="checkout.html" style="margin-top:10px;width:100%;justify-content:center">Checkout</a>`;
}
...
function render(f='All'){
  ...
  grid.innerHTML=items.map(p=>{
    ...
    return `<article class="card">
      <span class="badge">${p.tag||'NEW'}</span>
